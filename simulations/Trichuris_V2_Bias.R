...
sim_trial_Trichuris <- function(n_arms , N , mu,sigma,reductrate, rmonth, alpha , side,test,dropout,rr,rho)
{
  ...
  conclow <- wilcox.test(db_stage1$y_6m[db_stage1$treat %in% c("Low","Placebo")]~db_stage1$treat[db_stage1$treat %in% c("Low","Placebo")],alternative="greater")$p.value
  concmed<-wilcox.test(db_stage1$y_6m[db_stage1$treat %in% c("Medium","Placebo")]~db_stage1$treat[db_stage1$treat %in% c("Medium","Placebo")],alternative="greater")$p.value
  conc<-c(conclow,concmed)
  
  c(p.adjust(pval1,method="holm"),conc)
...
do_Trichuris = function(n_trials,n_arms , N , mu,sigma,reductrate0,reductrate1,reductrate2, rmonth, alpha , side1,test1,dropout,rr0,rr1,rr2,rho)
{
  ...
  mat<-matrix(unlist(results_list),ncol=4,byrow=T)
  
  pow<-c(apply(mat[,1:2]<=alpha,2,sum)/n_trials,sum((apply(mat[,1:2]<=alpha,1,sum)>0))/n_trials)
  
  true<-sim_trial_Trichuris (n_arms=n_arms , N=N*1000 , mu=mu,sigma=sigma,reductrate=reductrate, rmonth=rmonth, 
                       alpha=alpha , side=side,test=test,dropout=dropout,rr=rr,rho)
  
  trueconc<-true[3:4]
  #simul
  concord <- mat[,3:4]#matrix(unlist(lapply(results_list, function(element) element$conc)),ncol = 3, byrow = T)
  sdconcord<-apply(concord,2,sd,na.rm=TRUE)
  meanconcord<-apply(concord,2,mean,na.rm=TRUE)
  biasconcord<-meanconcord-trueconc
  
  return(c(pow,meanconcord,sdconcord,biasconcord,trueconc))
}
