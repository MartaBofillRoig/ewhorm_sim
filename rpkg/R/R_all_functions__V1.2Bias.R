...
do_pce_baseline = function(n_trials,n_arms = 4,N1, N2, mu_0m, mu_6m, mu_12m,  sg, rmonth, alpha1 , alpha , sel_scen, side,test,dropout,rr,bound)
{
  ...
  results_list <- future_map(1:n_trials, function(i) #sim_trial_pceind_test (n_arms = 4,N1 = 120 , N2 = 80, mu_0m = c(4,4,4,4),   
                                                    #                        mu_6m = c(4,4,4,4), mu_12m = c(4,4,4,4),  sg = sg, rmonth=1, 
                                                    #                        alpha1 = .3, alpha = .025, sel_scen=0, side=T,test="w1",
                                                    #                        dropout=.1,rr=c(0.1,0.1,0.1,0.4),bound=0)
                                                    #, 
  ...
  #sim_trial_pceind_test(n_arms = 4,N1 = 120 , N2 = 80, mu_0m = c(4,4,4,4),   mu_6m = c(4,4,4,2), mu_12m = c(4,4,4,2),  sg = sg, rmonth=1, 
  #alpha1 = .3, alpha = .025, sel_scen=0, side=T,test="w1",dropout=.1,rr=rep(0,4),bound=0)
  ...
  #concordCIBH <- matrix(unlist(lapply(results_list, function(element) element$concCIBH)),ncol = 3, byrow = T)
  #meanconcordCIBH<-apply(concordCIBH,2,mean,na.rm=TRUE)
  #coverageBH<-apply(t(concordCIBH)<trueconc,1,sum,na.rm=TRUE)/apply(!is.na(concordCIBH),2,sum)
  ...
  #lim<-500
  #par(mfrow=c(1,3))
  #plot(y = c(1: lim), x = concordCIinvn[1:lim,1], xlim = c(0, 1), ylim = c(0, lim),  ylab = "Simulation run", xlab = "Concordance", main = "Low dose",type="p")
  #points(x = rep(trueconc[1],lim), y = c(1: lim))
  ## Add connections with segments()
  #segments(y0 = c(1: lim),y1=c(1: lim), x0 = concordCIinvn[1: lim,1],x1 = rep(trueconc[1],lim))
#  
#  plot(y = c(1: lim), x = concordCIinvn[1:lim,2], xlim = c(0, 1), ylim = c(0, lim),  ylab = "Simulation run", xlab = "Concordance", main = "Median dose",type="p")
  #points(x = rep(trueconc[2],lim), y = c(1: lim))
  ## Add connections with segments()
  #segments(y0 = c(1: lim),y1=c(1: lim), x0 = concordCIinvn[1: lim,2],x1 = rep(trueconc[2],lim))
  
  #plot(y = c(1: lim), x = concordCIinvn[1:lim,3], xlim = c(0, 1), ylim = c(0, lim),  ylab = "Simulation run", xlab = "Concordance", main = "High dose",type="p")
  #points(x = rep(trueconc[3],lim), y = c(1: lim))
  ## Add connections with segments()
  #segments(y0 = c(1: lim),y1=c(1: lim), x0 = concordCIinvn[1: lim,3],x1 = rep(trueconc[3],lim))
  #dev.off()
  
  #return(c(armsel, condpow,pow,disjpow,pow_ma1,pow_ma2,pow_ma1a,recruittime1,recruittime2,trueconc,meanconcord,sdconcord,biasconcord,meanconcordMA1,conc1.,conc2.,biasCIup,biasCIlo))
...
#do_pce_baseline(n_trials=n_trials,n_arms = n_arms, N1=N1 , N2=N2, mu_0m=mu_0m, mu_6m=mu_6m, mu_12m=mu_12m, sg=sg, rmonth=rmonth, 
#                alpha1 = alpha1, alpha = alpha,sel_scen=sel_scen, side=side,test="w1",dropout=.1,rr=rep(0,4),bound=0)

o2<-do_pce_baseline (#mu_raw_0 = 650, sd_raw_0 = 575, r0_6=0, r1_6=0,r2_6=0,r3_6=0,r0_12=0,r1_12=0,r2_12=0,r3_12=0, rho = 0.5, 
           n_trials=10000,n_arms = 4,N1 = 120 , N2 = 80,mu_0m =c(10,10,10,10), mu_6m =c(10,10,10,10), mu_12m=c(10,10,10,10), sg=matrix(c(1,0,0,0,1,0,0,0,1), ncol=3),
           rmonth=1, alpha1=.1 , alpha=.025, sel_scen=1, side=1,test="w1",dropout=.1,rr=c(0,0,0,0),bound=0)#
  ...
  aa = #do_pce_baseline(n_trials=n_trials,n_arms = 4,N1 = N1, N2 = N2, mu_0m = m0,   mu_6m = m6, mu_12m = m12,  sg = mtr1, 
      #                  rmonth=rmonth, alpha1=alpha1 , alpha=alpha ,   sim_out=sim_out,sel_scen=sel_scen, side=side,test=test)
        do_pce_baseline(n_trials=n_trials,n_arms = 4,N1=N1 , N2=N2, mu_0m=m0, mu_6m=m6, mu_12m=m12, sg=mtr1,
                    rmonth=rmonth, alpha1=alpha1 , alpha=alpha , sel_scen=sel_scen, side=side,test=test,dropout=dropout,rr=rr,bound=bound)
...
#o1<-simul_res (mu_raw_0 = 650, sd_raw_0 = 575, r0_6=0, r1_6=0,r2_6=0,r3_6=0,r0_12=0,r1_12=0,r2_12=0,r3_12=0, rho = 0.5, 
#           n_trials=10000,n_arms = 4,N1 = 60 , N2 = 150,
#                rmonth=1, alpha1=.1 , alpha=.025, sim_out=1,sel_scen=0, side=1,test=4,dropout=.1,rr0=0,rr1=0,rr2=0,rr3=0,bound=0)#


o2<-simul_res (mu_raw_0 = 1838, sd_raw_0 =2565, r0_6=0, r1_6=0,r2_6=0,r3_6=0,r0_12=0,r1_12=0,r2_12=0,r3_12=0, rho = 0.5, 
               n_trials=10000,n_arms = 4,N1 = 60 , N2 = 150,
               alpha1=.1 , alpha=.025, sel_scen=0, side=1,test=0,dropout=.1,rr0=0.1,rr1=0.1,rr2=0.1,rr3=0.1,bound=0)#
